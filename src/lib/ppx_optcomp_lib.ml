...
module Meta_ast : sig
  ...
  let attr_mapper ~to_loc ~to_attrs ~replace_attrs ~env item =
    ...
    let is_our_attribute (attr, _) = let txt = attr.txt in Token.Directive.matches txt ~expected:"if" in
    let our_as, other_as = List.partition_tf (to_attrs item) ~f:is_our_attribute in
    match our_as with
    | [] -> Some item
    | [attr] ->
      (* Attribute.mark_as_handled_manually attr; *)
      begin match Interpreter.eval env (get_expr ~loc (snd attr)) with
...
let () =
  Driver.register_transformation "ppx_optcomp"
